...
if start_transition == nil then start_transition = false end
...
local function update ()
    gcs:send_text(1, "start_transition: " .. tostring(start_transition))
    if (start_transition==true and z_flag~=1)then
        init_c()
    end
    if (z_flag==1) then
        now=millis()
        ...
        --debugging
        gcs:send_text(1,"Controller Turned ON")
        
        --interpolation of inputs
        local y_d=interpolate(Y_d,(now-t_0)/1000,"sim")
        local u_d=interpolate(U_d,(now-t_0)/1000,"sim")
        --local y=interpolate(Y,(now-t_0)/1000,"log")
        --local u=interpolate(U,(now-t_0)/1000,"log")
        local y=state_log[#state_log]
        local u=input_rate_log[#input_rate_log]
        local k=interpolate(K,(now-t_0)/1000,"gain")
        --debugging
        gcs:send_text(1,"arrays interpolated")
        ...
        if (now-update_timestamp_elevator>update_interval_elevator or elevator_overide_flag==false) then
            ...
            elevator_pwm=1500+u_cmd_elevator_norm*500
