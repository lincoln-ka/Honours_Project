...
--input params
local sim_time_total=15.998
local sim_time_steps=17
...
local function interpolate (func,now_p)
    local x1=math.floor(now_p/sim_td)
    local x2=x1+1
    local x3=now_p/sim_td
    local w1=x3-x1
    local w2=x2-x3
...
local function update ()
    ...
    if (z_flag==1) then
        ...
        local y_d=interpolate(Y_d,now)
        local u_d=interpolate(U_d,now)
        local y=interpolate(Y,now)
        local u=interpolate(U,now)
        local k=interpolate(K,now)
